...
A project directory named `java-fix-local-variable-shadowing` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-local-variable-shadowing` directory which is created just now.
